...
contract SREAgency is ERC721 {
    ...
    constructor() 
        ERC721("SikanderRealEstateAgency", "SREAgency")
        public
    {
    ...
    function registerProperty(string memory _propertyNumber, string memory _propertyAddress, string memory _propertyURI, uint256 _propertyPrice) 
        ...
        returns (bool) 
    {
        require(msg.sender != address(0), "SREAgency: caller query of zero address");
        require(_propertyPrice > 0, "SREAgency: Invalid price");
    ...
    function changePropertyPrice(uint256 propertyId, uint256 propertyPrice)
        ...
        returns (bool)
    {
        require(_exists(propertyId), "SREAgency: Invalid propertyId, not registered");
        require(_isApprovedOrOwner(msg.sender, propertyId), "SREAgency: caller doesnot own the property");
        require(propertyPrice > 0, "SREAgency: Invalid price");
        ...
        returns (bool)
    {
        require(msg.sender != address(0), "SREAgency: caller query of zero address");
        require(_exists(propertyId), "SREAgency: Invalid propertyId, not registered");
        require(!_isApprovedOrOwner(msg.sender, propertyId), "SREAgency: caller cannot be approved, operator or owner itself");
        require(offerPrice > 0, "SREAgency: Invalid offer price");
        require(propertyOnSales[propertyId] == true, "SREAgency: mentioned property not on sale");
        ...
        returns (bool)
    {
        ...
        require(propertyBuyer.offerStatus == OfferStatus.Accepted, "SREAgency: this buyer's offer is not accepted");
        require(_buyer == propertyBuyer.buyer, "SREAgency: caller must be the accepted buyer");
        require(price == propertyBuyer.offerPrice, "SREAgency: Invalid offer price");
    ...
    function _processOffer(address owner, uint256 propertyId, uint256 index, OfferStatus _offerStatus) internal {
        require(owner != address(0), "SREAgency: caller query of zero address");
        require(_exists(propertyId), "SREAgency: Invalid propertyId, not registered");
        require(owner == ownerOf(propertyId), "SREAgency: caller must own the property");
